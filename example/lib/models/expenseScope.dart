...
class ExpenseModel extends Model {
  ...
  Map<String, Map<String, double>> calculateShares() {
    ...
    for (Map entry in _expenses) {
      ...
      tmpStats["Total Spends"]?[entry["person"]] = tmpStats["Total Spends"]?[entry["person"]] ?? 0 +  amount;
      for (MapEntry val in entry["shareBy"].entries) {
        tmpStats["Total Owe"]![val.key] = tmpStats["Total Owe"]![val.key]  ?? 0 +  double.parse(val.value);
        tmpStats["Net Owe"]![val.key] = tmpStats["Net Owe"]![val.key] ?? 0 +  double.parse(val.value);
      }
    }

    for (String u in _users) {
      tmpStats["Net Owe"]![u] = (tmpStats["Total Owe"]![u]  ?? 0 - tmpStats["Total Spends"]![u]!)!;
    }
  ...
  Map calculateCategoryShare() {
    ...
    for (Map entry in _expenses) {
      ...
      cShare[entry['category']] = cShare[entry['category']] ?? 0 +  double.parse(entry['amount']);
    }
